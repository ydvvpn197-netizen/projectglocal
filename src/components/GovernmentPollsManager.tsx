...
interface GovernmentPollsManagerProps {
  compact?: boolean;
}

export const GovernmentPollsManager: React.FC<GovernmentPollsManagerProps> = ({ 
  compact = false 
}) => {
  ...
  const [selectedLevel, setSelectedLevel] = useState<'local' | 'state' | 'national' | ''>('');
  ...
  const getAuthorityLevel = (authorityId: string) => {
    const authority = authorities.find(a => a.id === authorityId);
    return authority?.level || 'local';
  };
